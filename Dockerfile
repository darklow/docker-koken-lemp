...
RUN \
	...
	apt-get update && \
	apt-get -y install nginx mysql-client php5.6-fpm php5.6-mysql php5.6-curl php5.6-mcrypt graphicsmagick ffmpeg pwgen wget unzip
...
RUN \
	...
	sed -i -e "s/post_max_size\s*=\s*8M/post_max_size = 101M/g" /etc/php/5.6/fpm/php.ini && \
	...
	echo "env[DATABASE_URL] = \$DATABASE_URL" >> /etc/php/5.6/fpm/pool.d/www.conf && \
	...
	sed -i -e "s#listen\s*=\s*/run/php/php5\.6-fpm\.sock#listen = /run/php/php5\.6-fpm-images.sock#" /etc/php/5.6/fpm/pool.d/images.conf && \
	mkdir /run/php
